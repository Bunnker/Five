...
- 公共页面展示大吉、次吉、平；“较差”和“不利”合并显示为“注意”，并给出用大吉色普通配饰平衡的建议；
...
- 算法中的“较差”和“不利”只在内部数据中保留，公开页面统一显示为“注意”，表示今日建议降低使用比例，不使用警报视觉和恐吓文案；
- 用户已经穿了“注意”颜色时，可以建议用大吉色的丝巾、包、鞋、领带、耳饰或手机壳等普通配饰做小面积平衡；
...
| F02 | 今日颜色说明 | P0 | 公开网页 | 展示大吉、次吉、平和合并后的“注意” |
...
| 查看颜色 | 打开 F02 | 公开显示大吉、次吉、平与“注意”；内部算法仍保留五档 |
...
6. “注意”：合并展示较差与不利的两组颜色、减少使用说明和大吉色普通配饰平衡建议；
7. 今日怎么搭：单色、双色、三色缩略卡；
...
2. 较差与不利合并为紧凑的“注意”区域，不对用户展示两个负向档位名；
...
| 次吉 | `ci_ji` | 稳妥选择 |
| 平 | `ping` | 日常可穿 |
| 注意 | `jiao_cha` + `bu_li` | 今天减少大面积使用；已经穿了可用大吉色小配饰做平衡 |

动作规则：

- 大吉、次吉、平显示“查看穿法”，进入 F03 并定位到使用该档颜色的方案；
- “注意”不创建负面详情，只展示两组颜色、简短说明和普通配饰平衡建议；
- 页面底部统一提供“看看怎么搭”入口，进入 F03 默认方案；
- `较差`、`不利` 两个算法档位名必须保留在数据、后台和推算依据中，但不直接作为公开卡片标题。
...
公开网页、后台和公共 API 共用相同的内容快照和活跃版本。展示层合并“注意”不得改变内部日柱、五档、颜色、搭配配方或 `contentVersion`。
...
- 后台登录方案在开始开发前冻结，至少具备强密码、限流、会话过期和登录记录。
...
- 管理后台只有一个维护者账号，但仍执行登录保护、会话过期和审计日志；
...
- 已确认：公开显示大吉、次吉、平与合并后的“注意”；
- 已确认：“注意”只提示减少大面积使用，已穿时可用大吉色普通配饰做小面积平衡；

...
- `/today` 保留最长 60 秒动态共享缓存，并在命理日、时辰和民用午夜边界前提前失效；
- 内容采用八个状态，由一名维护者操作；大师在系统外查看并留下确认依据；
- 公共页面直接展示大吉、次吉、平、较差、不利五档，并给已经穿了较差或不利档颜色的用户提供大吉色普通配饰平衡建议；
- 日历使用固定答案表和黄金数据：工程侧机器比对 366 个命理日，大师书面复核连续 30 日及边界样本；
- 图片在开发或内容准备阶段离线批量生成，优先使用 Codex 调用 GPT Image 2，现有中转接口仅作可替换的备用通道；
- 每天准备 2 张必备图片，额度和质量允许时增加第 3 张；先完成 30 天并由大师确认，再考虑扩到一年；
...
3. 系统自动计算日期结果并执行排期任务；内容由维护者整理和检查、大师在系统外核对，再由维护者发布、下线或恢复。
...
- P0 的内容生效、缓存失效和页面内跨日均以 23:00 为边界。
...
| F07 | 分享选择 | P0 | 公开网页 | 系统分享、复制链接或下载海报 |
| F08 | 日签海报 | P0 | 公开网页 | 使用普通网页二维码 |
| F10A | 设置与帮助 | P0 | 公开网页 | 字号、反馈、协议、清除本地数据 |
| F11 | 全局状态 | P0 | 公开网页 | 加载、离线、错误、23:00 跨日 |
...
| 分享摘要 | 系统分享可用时调用，否则复制链接 | 带 `fortuneDate`、渠道参数和版本 |
...
P0 不在用户请求时临时生成图片。图片在开发或内容准备阶段离线批量生成并上传到自己的文件存储：

1. 系统生成结构化搭配配方；
2. 首选由 Codex 调用 GPT Image 2 批量生成；现有 Redar 中转接口作为可替换备用通道；
3. 每天先生成 2 张必备图，额度和质量允许时增加第 3 张；
4. 自动检查主色和禁止元素；
...
- 显示五档矩阵和颜色；
- 显示首页摘要、搭配公式、每日图片和海报模板预览；
- 由一名维护者逐项检查或退回到草稿；
- 记录操作者、时间、意见、大师确认依据和版本；
- 支持定时生效、立即撤回和回滚。
...
任一必审项未通过时，整个快照不得批准。提交后的内容、素材引用和版本号不得原地修改。

## 16.8 角色与权限

P0 只有一个后台账号，由项目维护者使用。该账号可以创建、编辑、提交、检查、排期、发布、撤回和回滚，但不能绕过机器校验、必审清单或大师确认依据。
...
- 发布前显示完整检查清单，未勾完不能继续；
- 大师确认依据缺失时，日历与五行相关内容不能进入 `approved`；
- 高风险动作要求再次输入当前内容版本或明确确认，避免误点；
- 后台登录与恢复遵守 ADR-0020：账号只通过空凭据库上的交互式命令首次创建；至少 16 位密码先换五分钟一次性挑战，再用未重放的六位 TOTP 建立会话；
...
- 首次初始化生成十个一次性恢复码。恢复第一阶段原子消费一个恢复码并立即使旧凭据和全部会话失效，第二阶段须在十分钟内设置新密码与新 TOTP，成功后只显示一次新的十个恢复码；
- 登录与恢复在正文解析前先按单向请求来源指纹做 PostgreSQL 限流；严格解析并规范化账号后、昂贵凭据校验前再做账号级限流，不存在的账号也按唯一账号当前哈希参数执行等价工作量校验；
- 登录、恢复、退出、限流、CSRF 拒绝、离线重置和紧急控制追加安全事件，至少保留 365 天且不记录任何凭据原文。

全局紧急停止与恢复都要求当前安全会话、可信 Origin、CSRF、fresh TOTP、非空原因、精确确认短语、开关 ETag 和 `Idempotency-Key`。停止短语为“停止全部公开内容”，恢复短语为“恢复全部公开内容”。开关关闭后，今日、指定日期、搭配、海报创建/查询和受控海报素材读取在源站统一返回 `503 PUBLIC_ACCESS_STOPPED`；帮助、反馈和健康检查保留。已经下载或外部转发的副本无法召回，正式 CDN 联动仍须在部署阶段验证。
...
```json
{
  ...
  "entry": {
    "type": "web_qr",
    "landingUrl": "https://example.com/d/2026-07-15?expectedContentVersion=fd-20260715-r3&channelId=organic"
  }
}
```

网页实例的 `entry.type` 为 `web_qr`，只返回 `landingUrl`。海报正文、日期或配方变化必须生成新的 `contentVersion`；只改变入口码、渠道标识或重新渲染时生成新的 `posterInstanceId`。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| `POST` | `/admin/api/v1/auth/password-challenges` | 以账号与密码创建五分钟一次性 TOTP 挑战 |
| `POST` | `/admin/api/v1/auth/sessions` | 以挑战与 TOTP 建立后台会话 |
| `GET/DELETE` | `/admin/api/v1/auth/session` | 查看当前会话与 CSRF 令牌，或退出当前会话 |
| `POST` | `/admin/api/v1/auth/logout-all` | 注销全部后台会话 |
| `POST` | `/admin/api/v1/auth/recovery-challenges` | 原子消费恢复码并开始十分钟两阶段恢复 |
| `POST` | `/admin/api/v1/auth/recovery-completions` | 启用新密码、新 TOTP 与十个新恢复码 |
...
| 401 | `AUTHENTICATION_FAILED` | 账号、密码、恢复码或未认证阶段动态码无效；对外统一文案 |
| 401 | `AUTH_CHALLENGE_EXPIRED` | 一次性密码挑战已过期或已消费 |
| 401 | `RECOVERY_CHALLENGE_EXPIRED` | 十分钟恢复挑战已过期或已消费 |
...
| 409 | `TOTP_REPLAYED` | 已认证的高风险操作重复使用已接受的 TOTP counter |
...
- 管理后台只有一个维护者账号，但仍执行强密码、TOTP、防重放、持久化限流、安全会话、CSRF 与追加审计；
- 安全事件至少保留 365 天，只保存请求编号、单向来源指纹和受限浏览器摘要，不保存密码、动态码、恢复码、挑战、会话、CSRF、原始 IP 或完整 User-Agent；
...
- 使用同一测试时刻打开公开网页和后台预览，`fortuneDate`、日干支、五行、内部五档和 `contentVersion` 完全一致；
- 网页相同 `lookId` 的标题、图片、配色比例和场景来自同一内容快照；
- 22:59、23:00、23:59、00:00 分别执行浏览器测试，业务日期和提示符合第 23.2 节；
...
- `/today`：`responseGeneratedAt` 表示缓存表示生成时的服务端结果；动态 `s-maxage` 最长 60 秒，并在命理日、时辰和民用午夜边界前提前失效；
- 内容状态：固定为 `draft`、`in_review`、`changes_requested`、`approved`、`scheduled`、`published`、`superseded`、`withdrawn`；
- 维护方式：一名维护者操作后台，大师在系统外确认并留下依据；
...
- 后台安全：Issue #31 已确认单维护者强密码 + TOTP、十个一次性恢复码、30 分钟闲置/12 小时绝对会话、全部会话撤销、关键安全记录和全局紧急开关，详见 ADR-0020。

已解除的开发阻塞：

- 单人后台登录、二次确认、恢复和紧急停止方案已于 2026-07-31 经维护者明确确认，可以按冻结契约开发。
...
- 冻结单人后台强密码、TOTP、恢复码、安全会话、安全记录与紧急停止，解除后台写功能的认证方案阻塞；服务器、域名、存储、CDN 和真实微信验收仍是公开上线阻塞。

...
> 本次修订：Issue #36 调整公开档位标签，页面直接显示“较差”“不利”，同时保持现有 OpenAPI wire contract 不变
...
6. “较差”和“不利”两张紧凑卡：分别展示档位名、颜色、减少使用说明和大吉色普通配饰平衡建议；
7. 今日怎么搭：单色、双色、三色缩略卡；
...
2. 较差与不利可位于同一紧凑区域，但必须使用 `algorithmLabel` 分别显示两个档位名、颜色组和五行关系；
